...
export default class PushNotificationService implements NotificationProvider {
  readonly code = "PUSH_NOTIFICATION"
  async notify(
    user: User,
    message: string,
    categoryCode: string
    ...
    try {
      ...
      console.log(`Failed sending push-notification to ${user.name}`, error)
    }
  }
}
